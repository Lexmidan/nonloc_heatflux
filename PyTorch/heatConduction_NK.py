...
def F_Newton_Krylov(T, para, cache, scalator):
    ...
    some_strange_coef=2610492
    F=(3/2*ne)*(T - T0)*Kb/dt + some_strange_coef*np.gradient(heatflux, x)
...
def newtonIteration(para, cache, SH, verbose):
    ...
    if SH:
        ...
        norm = np.linalg.norm(firstF*1e-5)
    ...
    def callbackfunc(x, f, norm=norm): 
        ...
        #numofiters+=1
        try: 
            residue = np.abs(f*norm/energy_init)
            slump = np.linalg.norm(f*norm)
        except TypeError:
            slump = np.linalg.norm(f(x)*norm)
    ...
    cache['T'] = scipy.optimize.newton_krylov(F_onevar, cache['T'], f_rtol=5e-12,iter=13, verbose=verbose, callback=callbackfunc)
...
def solve(para, SH=False,  verbose=False):
    ...
    print("If you use model, then alpha and beta doesnt participate in the scheme. Only heatflux")
    print(f'Use SH ? {SH}')
    start = time.time()
